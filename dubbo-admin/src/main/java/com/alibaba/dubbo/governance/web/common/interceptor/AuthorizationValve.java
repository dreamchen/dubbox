...
public class AuthorizationValve extends AbstractValve {
    ...
    private User getUser(String username) {
        ...
        Cookie cookie = new Cookie("locale", user.getLocale());
                ...
                if (user != null) {
                    ...
                    if (pwd != null && pwd.length() > 0) {
                        if (passwordDigest.equals(pwd)) {
                            ...
                            user = tool.getUserWithAllPrivilege(user.getId());
//                            request.getSession().setAttribute(WebConstants.CURRENT_USER_KEY, user);
                            return user;
                        }
                    }
...
//                            request.getSession().setAttribute(WebConstants.CURRENT_USER_KEY, user);
    ...
    private void setLogout(boolean logoutValue) {
        if (logoutValue) {
            request.getSession().setAttribute(WebConstants.CURRENT_USER_KEY, null);
            Cookie cookie = new Cookie("locale", null);
            cookie.setMaxAge(0);
            response.addCookie(cookie);
        }
