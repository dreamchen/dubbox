...
public class Tool {
     ...
     * @return
     */
    public UserExtend getUserWithAllPrivilege(Long uid) {
        UserExtend userExtend = (UserExtend) this.userService.findById(uid);
        String appPrivilege = "", servicePrivilege = "", managePrivilege = "";

        if (userExtend != null && !StringUtils.isBlank(userExtend.getRoleId())) {
            String[] roleIds = userExtend.getRoleId().split(",");
            List<Long> roleIdList = new ArrayList<Long>();
            for (String roleId : roleIds) {
                roleIdList.add(Long.parseLong(roleId));
            }
            List<Role> roleList = this.roleService.findRolesByIds(roleIdList);
            for (Role role : roleList) {
                appPrivilege = buildPrivilege(appPrivilege, role.getAppPrivilege(), ",");
                servicePrivilege = buildPrivilege(servicePrivilege, role.getServicePrivilege(), ",");
                managePrivilege = buildPrivilege(managePrivilege, role.getManagePrivilege(), ",");
            }
        }
        appPrivilege = buildPrivilege(appPrivilege, userExtend.getAppPrivilege(), ",");
        servicePrivilege = buildPrivilege(servicePrivilege, userExtend.getServicePrivilege(), ",");
        managePrivilege = buildPrivilege(managePrivilege, userExtend.getManagePrivilege(), ",");
        userExtend.setAppPrivilege(appPrivilege);
        userExtend.setServicePrivilege(servicePrivilege);
        userExtend.setManagePrivilege(managePrivilege);
        return userExtend;
    }
     ...
     * @return
     */
    public String[] getAllPrivilege(Long uid) {
        UserExtend userExtend = (UserExtend) this.userService.findById(uid);
        String appPrivilege = "", servicePrivilege = "", managePrivilege = "";
        ...
        return new String[]{appPrivilege, servicePrivilege, managePrivilege};
    }

    private String buildPrivilege(String sourcePrivilege, String targetPrivilege, String regex) {
        if (StringUtils.isBlank(targetPrivilege)) {
            return sourcePrivilege;
        }

        sourcePrivilege = StringUtils.isBlank(sourcePrivilege) ? "," : "," + sourcePrivilege + ",";
        if (sourcePrivilege.indexOf(",*,") > -1) {
            return "*";
        }
        for (String item : targetPrivilege.split(regex)) {
            if (item.equals("*")) {
                return "*";
            }
            if (sourcePrivilege.indexOf("," + item + ",") > -1) {
                continue;
            } else {
                sourcePrivilege += item + ",";
            }
        }
        return sourcePrivilege.substring(1, sourcePrivilege.length() - 1);
    }
}
